...
def main():
    ...
    if not os.path.exists('/data0/output/%s' %args.name):
        os.makedirs('/data0/output/%s' %args.name)
    ...
    # Data loading code
    img_paths = glob(r'/data0/BraTS2020/trainImage/*')
    mask_paths = glob(r'/data0/BraTS2020/trainMask/*')
    ...
    if val_args.mode == "GetPicture":
                        ...
                        for idx in range(output.shape[2]):
                            for idy in range(output.shape[3]):
                                ...
                                if output[i,2,idx,idy] > 0.5:
                                    ...
                                    rgbPic[idx, idy, 2] = 0
                        imsave('/data0/output/%s/'%args.name + rgbName,rgbPic)
                    break
        ...
        val_gt_path = '/data0/output/%s/'%args.name + "GT/"
    ...
    if val_args.mode == "Calculate":
        ...
        maskPath = glob("/data0/output/%s/" % args.name + "GT/*.png")
        pbPath = glob("/data0/output/%s/" % args.name + "*.png")
        if len(maskPath) == 0:
            print("请先生成图片!")
            return
