...
ALLOWED_EXTENSIONS = {'txt', 'pdf', 'png', 'jpg', 'jpeg', 'gif', 'npy'}  # 允许的上传文件类型
...
def upload():
    if request.method == 'POST':
        ...
        if f and allow_filename(f.filename):
            ...
            f.save(filepath)
            xianshi_img_name = filename +".png"
            print(filepath)
            img = predict.predicting(filepath,xianshi_img_name)
            
            # print(img)
            # img = np.array(Image.open('./upload/'+filename))
            outfilename = 'images/output/'+xianshi_img_name
            return render_template('upload_ok.html',outfilename = outfilename ,inputfilename='images/upload/'+xianshi_img_name)
